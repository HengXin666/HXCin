...
- [本库的输入, 和Java原生输入混用示例](./src/test/java/Test03.java) (有一个缺点是, 因为焦点有时候在`swing`窗口, 因此用户原生输入的时候, 还需要点击一下控制台, 来获取焦点...)
...
因为本库是基于`swing`窗口的按键事件的, 因此退出的时候需要关闭窗口、或者调用`System.exit(0);`强制停止程序.
...
> [!TIP]
> 我劝你也是少用, 最好不用CSDN. 这个广告比片网还多的垃圾网站 (和世界运动校园坐一桌!), 里面全是复制粘贴、可信度极低的东西, AI要是联网搜索到CSDN的东西, 包是智商剧降, 错误连篇!